<template>
  <div class="max-w-7xl mx-auto h-full flex flex-col">
            ...
            <div>
              ...
              <div class="flex flex-col gap-3">
                <label class="flex items-center gap-3 cursor-pointer">
                  <input v-model="theme" type="radio" value="dark" class="accent-[#409EFF]" @change="updateTheme" />
                  <span class="text-[var(--foreground)]">深色主题</span>
                </label>
                <label class="flex items-center gap-3 cursor-pointer">
                  <input v-model="theme" type="radio" value="light" class="accent-[#409EFF]" @change="updateTheme" />
                  <span class="text-[var(--foreground)]">浅色主题</span>
                </label>
                <label class="flex items-center gap-3 cursor-pointer">
                  <input v-model="theme" type="radio" value="system" class="accent-[#409EFF]" @change="updateTheme" />
          ...
          <div class="space-y-4">
            <div>
              ...
              <div class="flex items-center gap-3">
                <input v-model="autoCheckUpdate" type="checkbox" class="accent-[#409EFF]"
                  @change="updateAutoCheckUpdate" />
                <span class="text-[var(--foreground)]">应用启动时自动检查更新</span>
              </div>
            </div>

            <div class="flex items-center gap-3">
              <button @click="checkForUpdates" class="btn-primary text-sm">
                立即检查更新
              </button>
              <span class="text-xs text-[var(--muted-text)]">上次检查: {{ lastCheckTime || '从未检查' }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <!-- 更新信息弹窗 -->
    <UpdateDialog v-if="updateInfo" :updateInfo="updateInfo" @close="updateInfo = null" />
  </div>
</template>
